...
```sh
./target/release/guillotine-chain -h
```
...
```sh
./target/release/guillotine-chain --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/guillotine-chain purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/guillotine-chain -ldebug --dev
```
...
$ ./target/release/guillotine-chain --dev --base-path ./my-chain-state/
...
A hosted version is also available on [IPFS (redirect) here](https://dotapps.io/) or [IPNS (direct) here](ipns://dotapps.io/?rpc=ws%3A%2F%2Flocalhost%3A9944#/explorer).
...
The runtime in this project is constructed using many FRAME pallets that ship with the [core Substrate repository](https://github.com/paritytech/substrate/tree/master/frame) and some pallet that is [defined in the `pallets`](./pallets/) directory.
